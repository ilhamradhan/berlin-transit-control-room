...
def build_and_publish_release(releases_root, manifest_path, *, project_dir, dbt_bin):
    ...
    try:
        ...
        for command in commands:
            ...
            if completed.returncode:
                raise ReleaseBuildError(
                    f"dbt {' '.join(command)} failed ({completed.returncode}): {completed.stdout}{completed.stderr}"
                )
        ...
        payload = json.dumps({"path": str(database), "verified_read_only": True}, sort_keys=True).encode()
        guarded_atomic_write(manifest.parent, Path(f".{manifest.name}.tmp"), [payload])
        os.replace(manifest.parent / f".{manifest.name}.tmp", manifest)
...
def main(argv=None):
    ...
    parser.add_argument("command", nargs="?", choices=("static", "realtime", "maintain", "metrics", "release"))
