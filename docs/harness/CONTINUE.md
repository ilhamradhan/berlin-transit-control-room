...
> Continue TransitOps Berlin from the current stage. Read `docs/harness/CONTINUE.md`, inspect the repository and current stage record, then follow the resume workflow. Do not skip the stage gate. Do not commit or push unless I explicitly ask.
...
>
> Load `plan` and write a Stage 1 execution plan under `.hermes/plans/`. Use `spike` for the measured lightweight Airflow experiment. Suggest `domain-modeling` only if GTFS terms or source contracts remain unclear. Use `architecture-diagram` only after the measured topology is accepted.
>
> The Stage 1 plan must cover the pinned environment proposal, non-destructive preflight, VBB static and realtime sample validation, VBB feed-status evidence, source contracts, domain glossary, sample DAG, Airflow memory and disk measurements, architecture diagram, tests, and the Stage 1 record. Include exact files, commands, expected evidence, stop conditions, and rollback or cleanup for the disposable spike.
>
...
>
> Continue only the next incomplete task in Stage [N]. Use the smallest matching skill set and record a skill as used only if it changes an artifact or action. Run the task's verification, update the stage record with concise evidence, and stop if a decision or unsafe side effect needs my input. Do not start Stage [N+1]. Do not commit or push unless I explicitly ask.
